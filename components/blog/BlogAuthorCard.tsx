...
export default function BlogAuthorCard({ author, authorDesignation, authorAvatar, authorConsultationUrl, socialLinks }: BlogAuthorCardProps) {
  return (
    <div className="rounded-lg bg-[#A687EF]/50 p-3 sm:p-4">
      ...
      {/* Mobile: keep current horizontal layout; Desktop: stack vertically like design */}
      <div className="flex items-start gap-4 text-left md:flex-col md:items-center md:text-center md:gap-5">
        ...
        <div className="flex-1 min-w-0 md:w-full md:mt-3">
          <p className="text-sm sm:text-base font-bold text-gray-900 truncate text-left md:text-center">
            {author} <CheckCircle2 className="inline text-blue-500" size={16} />
          </p>
          {authorDesignation && (
            <p className="text-xs text-gray-700 mt-0.5 md:mt-1 md:text-center">({authorDesignation})</p>
          )}

          <div className="mt-2 sm:mt-3 md:mt-4 md:w-full">
